#!/usr/bin/env python3
"""
Topic 3 Keyword Comparison: HDBSCAN vs Variable K-means
Creates a clean table comparing keywords for Topic 3 across queries
"""
...
def main():
    ...
    # Build comparison table for Topic 3
    results = []
    for query in common_queries:
        hdbscan_keywords = get_topic_keywords(hdbscan_path, query, 3)
        variable_k_keywords = get_topic_keywords(variable_k_path, query, 3)
    ...
    df.to_csv('topic3_keyword_comparison.csv', index=False)
    
    print(f"✓ Analyzed {len(common_queries)} queries for Topic 3")
    print(f"✓ Table saved as: topic3_keyword_comparison.csv")
    print(f"\nFirst 5 rows:")
