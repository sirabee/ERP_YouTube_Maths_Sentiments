#!/usr/bin/env python3
"""
Simple Topic 0 Keyword Comparison: HDBSCAN vs Variable K-means
Creates a clean table comparing keywords for Topic 0 across queries
"""
...
def get_topic0_keywords(base_path, query, path_pattern):
    """Extract Topic 0 keywords for a query."""
    ...
    try:
        ...
        keywords_str = topic0.iloc[0]['Representation']
...
def main():
    ...
    for query in common_queries:
        hdbscan_keywords = get_topic0_keywords(hdbscan_path, query, "hdbscan")
        variable_k_keywords = get_topic0_keywords(variable_k_path, query, "variable_k")
    ...
    df.to_csv('topic0_keyword_comparison.csv', index=False)
    
    print(f"✓ Analyzed {len(common_queries)} queries")
    print(f"✓ Table saved as: topic0_keyword_comparison.csv")
    print(f"\nFirst 5 rows:")
