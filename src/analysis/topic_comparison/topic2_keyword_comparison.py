#!/usr/bin/env python3
"""
Topic 2 Keyword Comparison: HDBSCAN vs Variable K-means
Creates a clean table comparing keywords for Topic 2 across queries
"""
...
def main():
    ...
    # Build comparison table for Topic 2
    results = []
    for query in common_queries:
        hdbscan_keywords = get_topic_keywords(hdbscan_path, query, 2)
        variable_k_keywords = get_topic_keywords(variable_k_path, query, 2)
    ...
    df.to_csv('topic2_keyword_comparison.csv', index=False)
    
    print(f"✓ Analyzed {len(common_queries)} queries for Topic 2")
    print(f"✓ Table saved as: topic2_keyword_comparison.csv")
    print(f"\nFirst 5 rows:")
