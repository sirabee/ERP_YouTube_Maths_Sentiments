#!/usr/bin/env python3
"""
XLM-RoBERTa Clean Model Agreement Analysis
Evaluates the clean XLM-RoBERTa model's agreement with manual annotations.
Creates comprehensive metrics similar to YouTube BERT evaluation.
...
class XLMRoBERTaAgreementAnalyzer:
    def __init__(self):
        ...
        print("XLM-RoBERTa Clean Model Agreement Analysis")
    ...
    def load_xlm_roberta_results(self):
        """Load XLM-RoBERTa clean model results."""
        results_path = "/Users/siradbihi/Desktop/MScDataScience/ERP_YouTube_Maths_Sentiments/results/models/sentiment_analysis/xlm_roberta_clean_20250816_132139/enhanced_comments.csv"
        
        print(f"Loading XLM-RoBERTa results...")
        df_model = pd.read_csv(results_path)
        
        print(f"Loaded {len(df_model):,} XLM-RoBERTa predictions")
        return df_model
    
    def map_predictions_to_annotations(self, df_manual, df_model):
        """
        Map XLM-RoBERTa predictions to manual annotations by matching comment text.
        
        Note: This assumes the manual annotations were created from a subset of the same
        dataset that XLM-RoBERTa processed.
        """
        ...
        for _, manual_row in manual_comments.iterrows():
            ...
            if len(model_matches) > 0:
                ...
                matched_data.append({
                    ...
                    'model_analysis_id': model_row['analysis_id'],
                    ...
                    'model_sentiment': model_row['xlm_sentiment'],
                    'model_confidence': model_row['xlm_confidence'],
                    'model_journey': model_row['learning_journey'],
                    'progression_type': model_row['progression_type'],
    ...
    def calculate_agreement_metrics(self, matched_df):
        ...
        metrics = {
            ...
            'model_name': 'xlm_roberta_clean',
            'model_details': {
                'model_path': 'AmaanP314/youtube-xlm-roberta-base-sentiment-multilingual',
                'aggregation_method': 'enhanced_clean_with_contradiction_detection',
                'features': [
                    'spacy_sentence_segmentation',
                    'contradiction_marker_detection',
                    'five_type_progression_classification',
            ...
            **metrics,
            ...
            'performance_summary': {
                ...
                'benchmark_comparison': {
                    'youtube_bert_accuracy': 0.72,  # From reference
                    'improvement_over_youtube_bert': round(
                        metrics['sentiment_agreement']['accuracy'] - 0.72, 4
                    ),
                    'twitter_roberta_accuracy': 0.37,  # From previous analysis
            ...
            'methodology_notes': {
                ...
                'code_version': 'xlm_roberta_clean_enhanced',
                'aggregation_confidence': 0.7  # For learning journeys
            }
        }
    ...
    def save_results(self, comprehensive_metrics, matched_df):
        ...
        metrics_file = output_dir / f"xlm_roberta_clean_agreement_metrics_{self.timestamp}.json"
        with open(metrics_file, 'w', encoding='utf-8') as f:
            json.dump(comprehensive_metrics, f, indent=2, ensure_ascii=False)
        
        # Save detailed matched data
        matched_file = output_dir / f"xlm_roberta_clean_matched_annotations_{self.timestamp}.csv"
    ...
    def run_agreement_analysis(self):
        ...
        try:
            ...
            df_model = self.load_xlm_roberta_results()
            ...
            print(f"vs YouTube BERT: {perf['improvement_over_youtube_bert']:+.3f}")
...
def main():
    ...
    analyzer = XLMRoBERTaAgreementAnalyzer()
