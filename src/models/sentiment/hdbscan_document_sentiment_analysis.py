#!/usr/bin/env python3
"""
Sentiment Analysis Integration with Existing BERTopic Results
processing 34,024 comments with topic assignments
"""
...
class BERTopicSentimentIntegration:
    def __init__(self, comments_file=None):
        """Initialize sentiment analysis pipeline for existing BERTopic results."""
        ...
        self.output_dir = f"sentiment_analysis_{self.timestamp}"
    ...
    def load_bertopic_results(self):
        """Load the existing BERTopic analysis results."""
        print(f"Loading BERTopic results from: {self.comments_file}")
        
        if not os.path.exists(self.comments_file):
            raise FileNotFoundError(f"BERTopic results file not found: {self.comments_file}")
        
        df = pd.read_csv(self.comments_file)
        print(f"Loaded {len(df):,} comments with topic assignments")
    ...
    def process_sentiment_batch(self, texts, batch_size=16):
        ...
        for i in tqdm(range(0, len(texts), batch_size), desc="Processing sentiment"):
            ...
            try:
                ...
                for _ in batch:
                    sentiments.append({
                        'sentiment_label': 'NEUTRAL',  # More appropriate default
                        'sentiment_score': 0.5
                    })
    ...
    def analyze_sentiment_by_topic(self, df):
        ...
        return self.analyze_sentiment_by_group(df, 'topic', 'topic')
    ...
    def create_topic_sentiment_mapping(self, df):
        """Create automated topic-to-aspect mapping based on sentiment patterns."""
        print("Creating topic-sentiment aspect mapping...")
    ...
    def create_visualizations(self, df, topic_sentiment_pct, query_sentiment_pct):
        ...
        plt.title('Overall Sentiment Distribution in YouTube Math Education Comments')
        plt.savefig(os.path.join(viz_dir, "overall_sentiment_distribution.png"), dpi=300, bbox_inches='tight')
        plt.close()
        
        # 2. Topic-level sentiment heatmap (top 20 topics)
        if len(topic_sentiment_pct) > 0:
            plt.figure(figsize=(12, 8))
            top_topics = topic_sentiment_pct.head(20)
            sns.heatmap(top_topics, annot=True, fmt='.1f', cmap='RdYlBu_r', center=50)
            plt.title('Sentiment Distribution by Topic (Top 20 Topics)')
            plt.xlabel('Sentiment')
            plt.ylabel('Topic ID')
            plt.tight_layout()
            plt.savefig(os.path.join(viz_dir, "topic_sentiment_heatmap.png"), dpi=300, bbox_inches='tight')
            plt.close()
        ...
        if len(query_sentiment_pct) > 0:
            ...
            plt.title('Sentiment Distribution by Search Query (Top 15 Queries)', fontsize=14)
            plt.xlabel('Search Query', fontsize=12)
            plt.ylabel('Percentage', fontsize=12)
            plt.xticks(rotation=45, ha='right')
            plt.legend(title='Sentiment', bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.tight_layout()
            plt.savefig(os.path.join(viz_dir, "query_sentiment_stacked.png"), dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def generate_academic_report(self, df, topic_analysis):
        ...
        report_path = os.path.join(self.output_dir, "academic_report.txt")
...
"""
        ...
        if len(topic_analysis) > 0 and not topic_analysis.empty:
            ...
            report += f"\nMost positive topic: Topic {most_positive['topic']} (score: {most_positive['avg_sentiment_score']:.3f})\n"
            report += f"Most negative topic: Topic {most_negative['topic']} (score: {most_negative['avg_sentiment_score']:.3f})\n"
        ...
        report += f"\n=== EDUCATIONAL INSIGHTS ===\n"
        report += f"This analysis reveals sentiment patterns in mathematical education discourse.\n"
        report += f"The integration of BERTopic themes with sentiment analysis enables\n"
        report += f"identification of specific educational aspects that generate positive\n"
        report += f"or negative emotional responses from learners.\n"
    ...
    def run_complete_analysis(self):
        """Execute the complete sentiment analysis integration."""
        print("=" * 70)
        print("SENTIMENT ANALYSIS INTEGRATION WITH BERTOPIC RESULTS")
        print("=" * 70)
        ...
        try:
            # Step 1: Load existing BERTopic results
            df = self.load_bertopic_results()
            ...
            enhanced_file = os.path.join(self.output_dir, "comments_with_topics_and_sentiment.csv")
            ...
            topic_sentiment_pct.to_csv(os.path.join(self.output_dir, "topic_sentiment_percentages.csv"))
            query_sentiment_pct.to_csv(os.path.join(self.output_dir, "query_sentiment_percentages.csv"))
            topic_analysis.to_csv(os.path.join(self.output_dir, "topic_sentiment_analysis.csv"), index=False)
            ...
            print("\n" + "=" * 70)
            print("SENTIMENT ANALYSIS INTEGRATION COMPLETED")
            print("=" * 70)
            print(f"Results saved in: {self.output_dir}/")
            print(f"Enhanced dataset: comments_with_topics_and_sentiment.csv")
            print(f"Academic report: academic_report.txt")
...
def main():
    ...
    analyzer = BERTopicSentimentIntegration()
