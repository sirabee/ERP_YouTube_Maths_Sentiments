#!/usr/bin/env python3
"""
Variable K-means Per-Sentence Sentiment Analysis for Educational Comments
Implements sentence-level analysis for Variable K-means BERTopic results
Addresses learning journey detection and sentiment transitions in mathematical education
"""
...
class VariableKMeansSentenceLevelAnalyzer:
    def __init__(self, comments_file=None):
        """Initialize Variable K-means sentence-level sentiment analyzer."""
        ...
        self.output_dir = f"/Users/siradbihi/Desktop/MScDataScience/YouTube_Mathematics_Sentiment_Analysis/Sentiment_Analysis/Variable_K_Means/variable_k_sentence_sentiment_analysis_{self.timestamp}"
        
        # Setup device
        self.device = "mps" if torch.backends.mps.is_available() else "cpu"
        print(f"Using device: {self.device}")
        print("Variable K-means Per-Sentence Analysis Initialization")
        
        # Initialize sentiment pipeline
        print("Loading Twitter RoBERTa sentiment model for Variable K-means analysis...")
    ...
    def process_dataset(self):
        ...
        print(f"Processing {len(df):,} Variable K-means comments with sentence-level analysis...")
        
        results = []
        for idx, comment in tqdm(enumerate(df['comment_text']), total=len(df), desc="Analyzing Variable K-means comments"):
    ...
    def create_analysis_report(self, enhanced_df, sentence_df):
        ...
        report_path = f"{self.output_dir}/variable_k_sentence_analysis_report.txt"
...
This Variable K-means sentence-level analysis reveals {learning_journeys} comments showing
...
"""
        ...
        print(f"Variable K-means analysis report saved: {report_path}")
    ...
    def create_visualizations(self, enhanced_df, sentence_df):
        ...
        plt.title('Variable K-means: Sentiment Progression Patterns')
        ...
        plt.title('Variable K-means: Final Sentence-Level Sentiment Distribution')
        plt.ylabel('Number of Comments')
        
        plt.tight_layout()
        plt.savefig(f"{viz_dir}/variable_k_sentiment_progression_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if len(enhanced_df[enhanced_df['sentiment_progression'] == 'learning_journey']) > 0:
            ...
            plt.title('Variable K-means: Distribution of Sentence Count in Learning Journey Comments')
            plt.grid(axis='y', alpha=0.3)
            
            plt.tight_layout()
            plt.savefig(f"{viz_dir}/variable_k_learning_journey_analysis.png", dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def create_comprehensive_visualizations(self):
        ...
        print("Creating comprehensive Variable K-means visualizations...")
        try:
            import subprocess
            import sys
    ...
    def run_complete_analysis(self):
        """Execute complete Variable K-means sentence-level sentiment analysis."""
        print("=" * 80)
        print("VARIABLE K-MEANS SENTENCE-LEVEL SENTIMENT ANALYSIS")
        ...
        try:
            ...
            enhanced_df.to_csv(f"{self.output_dir}/variable_k_comments_with_sentence_sentiment.csv", index=False)
            sentence_df.to_csv(f"{self.output_dir}/variable_k_sentence_level_details.csv", index=False)
            ...
            print("\n" + "=" * 80)
            print("VARIABLE K-MEANS SENTENCE-LEVEL ANALYSIS COMPLETED SUCCESSFULLY")
            print("=" * 80)
            print(f"Results saved in: {self.output_dir}/")
            print("Key files:")
            print("- variable_k_comments_with_sentence_sentiment.csv (enhanced dataset)")
            print("- variable_k_sentence_level_details.csv (sentence-by-sentence breakdown)")
            print("- variable_k_sentence_analysis_report.txt (comprehensive analysis)")
            print("- visualizations/ (Variable K-means charts and graphs)")
...
def main():
    """Main execution function for Variable K-means sentence-level analysis."""
