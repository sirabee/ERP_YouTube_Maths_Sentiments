#!/usr/bin/env python3
"""
Variable K-means Per-Sentence Sentiment Analysis for Educational Comments - YouTube BERT Version
Implements sentence-level analysis for Variable K-means BERTopic results
Uses YouTube-trained BERT model for improved educational sentiment analysis
"""
...
class VariableKMeansSentenceLevelAnalyzer:
    def __init__(self, comments_file=None):
        """Initialize Variable K-means sentence-level sentiment analyzer with YouTube BERT."""
        if comments_file is None:
            # Extract data from original sentiment analysis results
            base_path = "/Users/siradbihi/Desktop/MScDataScience/ERP_YouTube_Maths_Sentiments"
            comments_file = f"{base_path}/results/models/sentiment_analysis/variable_k_sentence_sentiment_analysis_20250731_010046/variable_k_sentence_level_details.csv"
        
        self.comments_file = comments_file
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Updated output directory for current repository
        base_output = "/Users/siradbihi/Desktop/MScDataScience/ERP_YouTube_Maths_Sentiments/results/models/sentiment_analysis"
        self.output_dir = f"{base_output}/youtube_bert_sentence_sentiment_analysis_{self.timestamp}"
        
        # Setup device
        self.device = "mps" if torch.backends.mps.is_available() else "cpu"
        print(f"Using device: {self.device}")
        print("Variable K-means Per-Sentence Analysis with YouTube BERT Initialization")
        
        # Initialize sentiment pipeline with YouTube BERT
        print("Loading YouTube BERT sentiment model for improved educational analysis...")
        ...
        try:
            ...
            print("SpaCy model loaded successfully for YouTube BERT sentence segmentation.")
        except OSError:
    ...
    def analyze_sentence_sentiment(self, sentence):
        """Analyze sentiment of a single sentence using YouTube BERT."""
        try:
    ...
    def process_dataset(self):
        """Process the Variable K-means dataset with YouTube BERT sentence-level analysis."""
        print(f"Loading original analysis results: {self.comments_file}")
        
        try:
            original_df = pd.read_csv(self.comments_file)
        except FileNotFoundError:
            print(f"Error: Could not find dataset at {self.comments_file}")
            print("Please check the file path or ensure the original analysis has been run.")
            return None, None
        
        # Extract unique comments from sentence-level data
        df = original_df.groupby('comment_id').agg({
            'original_text': 'first'
        }).reset_index()
        df = df.rename(columns={'original_text': 'comment_text'})
        
        print(f"Extracted {len(df):,} unique comments for YouTube BERT analysis")
        
        df['comment_text'] = df['comment_text'].astype(str).fillna('')
        df = df[df['comment_text'].str.len() > 0]
        
        print(f"Processing {len(df):,} comments with YouTube BERT sentence-level analysis...")
        
        results = []
        for idx, comment in tqdm(enumerate(df['comment_text']), total=len(df), desc="Analyzing with YouTube BERT"):
    ...
    def create_analysis_report(self, enhanced_df, sentence_df):
        """Create comprehensive YouTube BERT analysis report."""
        os.makedirs(self.output_dir, exist_ok=True)
        
        report_path = f"{self.output_dir}/youtube_bert_sentence_analysis_report.txt"
...
"""
        ...
        report += f"""
=== YOUTUBE BERT EDUCATIONAL INSIGHTS ===
This YouTube BERT analysis reveals {learning_journeys} comments showing
...
- Trained specifically on YouTube comment patterns
- Better understanding of educational discourse
- Improved accuracy in detecting learning journeys
- More appropriate sentiment classification for questions and discussions
...
"""
        ...
        print(f"YouTube BERT analysis report saved: {report_path}")
    
    def create_visualizations(self, enhanced_df, sentence_df):
        """Create visualizations for YouTube BERT sentence-level analysis."""
        ...
        plt.title('YouTube BERT: Sentiment Progression Patterns')
        ...
        plt.title('YouTube BERT: Final Sentence-Level Sentiment Distribution')
        plt.ylabel('Number of Comments')
        
        plt.tight_layout()
        plt.savefig(f"{viz_dir}/youtube_bert_sentiment_progression_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if len(enhanced_df[enhanced_df['sentiment_progression'] == 'learning_journey']) > 0:
            ...
            plt.title('YouTube BERT: Distribution of Sentence Count in Learning Journey Comments')
            plt.grid(axis='y', alpha=0.3)
            
            plt.tight_layout()
            plt.savefig(f"{viz_dir}/youtube_bert_learning_journey_analysis.png", dpi=300, bbox_inches='tight')
            plt.close()
        
        print(f"YouTube BERT visualizations saved to: {viz_dir}")
    
    def run_complete_analysis(self):
        """Execute complete YouTube BERT sentence-level sentiment analysis."""
        print("=" * 80)
        print("YOUTUBE BERT SENTENCE-LEVEL SENTIMENT ANALYSIS")
        print("Educational Comments - YouTube Mathematics Content")
        print("Improved Model: rahulk98/bert-finetuned-youtube_sentiment_analysis")
        print("=" * 80)
        ...
        try:
            ...
            # Save YouTube BERT results
            enhanced_df.to_csv(f"{self.output_dir}/youtube_bert_comments_with_sentence_sentiment.csv", index=False)
            sentence_df.to_csv(f"{self.output_dir}/youtube_bert_sentence_level_details.csv", index=False)
            ...
            print("\n" + "=" * 80)
            print("YOUTUBE BERT SENTENCE-LEVEL ANALYSIS COMPLETED SUCCESSFULLY")
            print("=" * 80)
            print(f"Results saved in: {self.output_dir}/")
            print("Key files:")
            print("- youtube_bert_comments_with_sentence_sentiment.csv (enhanced dataset)")
            print("- youtube_bert_sentence_level_details.csv (sentence-by-sentence breakdown)")
            print("- youtube_bert_sentence_analysis_report.txt (comprehensive analysis)")
            print("- visualizations/ (YouTube BERT charts and graphs)")
            
            # Print comparison note
            print("\n" + "=" * 80)
            print("COMPARISON WITH ORIGINAL TWITTER MODEL")
            print("=" * 80)
            print("To compare with original results, see:")
            print("- /results/models/sentiment_analysis/variable_k_sentence_sentiment_analysis_20250731_010046/")
...
def main():
    """Main execution function for YouTube BERT sentence-level analysis."""
